# hifi-design skill — verbatim capture

**Captured:** 2026-08-10 via `mcp__claude-design__read_design_skill { skill: "hifi-design" }`.
Live source is that tool. Fetch it when the work is polished product screens, mockups or prototypes.

---

# Hi-fi design — the high-fidelity design process

Create a high-fidelity, polished design.

Follow this general design process (track the steps — your host's todo list, or a scratch note):
(1) ask questions, (2) find existing UI kits and collect design context — copy ALL relevant components and read ALL relevant examples; ask the user if you can't find them, (3) start your file with assumptions + context + design reasoning (as if you are a junior designer and the user is your manager), with placeholders for the designs, and show it to the user early, (4) build out the designs and show the user again ASAP; append some next steps, (5) check, verify and iterate on the design ("See your work — the verify loop" in the Claude Design prompt).

Good hi-fi designs do not start from scratch — they are rooted in existing design context. Ask the user for design context: screenshots of existing UI, brand assets, Figma exports, a link to their codebase — or point them at Claude Design's Import menu (claude.ai/design), where they can link a codebase, provide screenshots or Figma links, or link another project. You MUST spend time trying to acquire design context, including components. If you cannot find it, ask the user for it. Mocking a full product from scratch is a LAST RESORT and will lead to poor design. If stuck, `list_files` the bound design-system project and read its examples — be proactive! Some designs may need multiple design systems — get them all. If the design system ships starter components (device frames and the like), copy them in with `copy_files` for high-quality scaffolding for free.

When showing multiple design options on one page, decide between (a) a single full-size responsive prototype, or (b) a vertical stack of anchored option cards. Choose based on how design-y vs prototype-y the ask is, how many options there are, and how big each is. For (b):

Present multiple design options as a vertical stack of turns — each turn of options is its own `<section>`, newest turn at the **top**, and every option gets a stable `{turn}{letter}` id (`1a`, `1b`, `2a`…) that the user references back in chat and you cross-link between turns. Always include `<meta name="design_doc_mode" content="canvas">` in `<helmet>`, so the user can freely zoom out on designs wider than the viewport.
...
<helmet data-dc-atomics
  ><meta name="design_doc_mode" content="canvas" /><style>
